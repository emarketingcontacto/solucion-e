...
<div class="group relative bg-gray-900/50 border border-gray-800 rounded-3xl p-1 overflow-hidden hover:border-brand/50 transition-all duration-500">
    ...
    <div class="relative bg-gray-950 rounded-[22px] overflow-hidden">
        ...
        <div class="h-64 overflow-hidden">
            @if($servicio->imagen)
                <img src="{{ asset('storage/' . $servicio->imagen) }}"
                    alt="Servicio de {{ $servicio->nombre }} -Solucion-e"
                    width="600"
                    height="400"
                    loading="lazy"
                    decoding="async"
        ...
        <div class="p-8">
            ...
            @endphp
            <a href="{{route($url)}}"
                aria-label="Saber más sobre {{ $servicio->nombre }}"
                class="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-white hover:text-brand transition">
                Saber más
